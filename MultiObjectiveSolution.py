...
payoff_table = OP.create_payoff_table(number_obj_functions = number_of_obj_functions)
...
for i in range(0,ModelP.number_of_obj_functions-1):
    while i_counter[i] < number_of_grid_points[i]:

        # Solve problem P
        OP.solveProblemP(ModelP, ranges, grid_points[1][i_counter[i]])

        # If feasible
        if ModelP.SingleObjectiveModel.status == gb.GRB.Status.OPTIMAL:
            # Safe Solution
            nonDominatedSolutions.append(ModelP.saveSolution())
...
for i in range(len(nonDominatedSolutions)):
    print(nonDominatedSolutions[i].objVal)
