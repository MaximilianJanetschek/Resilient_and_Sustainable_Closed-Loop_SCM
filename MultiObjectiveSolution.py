...
while i_counter[2] < number_of_grid_points[2]:
    while i_counter[1] < number_of_grid_points[1]:
        # Solve problem P
        solution, S, feasible_status, established_locations, supply_links, suppliers_op = OP.solveProblemP(ModelSCM, objectives, ranges, grid_points[1][i_counter[1]], grid_points[2][i_counter[2]], len(objectives), Variable)
        ...
        if feasible_status:
            ...
            suppliers_oppened.append(suppliers_op)
...
pickle.dump(suppliers_oppened, open("suppliers_oppend.p", "wb"))

test_matrix = np.matrix([[1,1/3,1/9],[3,1,5],[9,1/5,1]])
